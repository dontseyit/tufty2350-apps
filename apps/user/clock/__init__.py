...
APP_DIR = "/system/apps/user/clock"
...
# Credentials come from /system/secrets.py - the file you can edit directly on
# the badge's USB drive. Put /system on the import path (and drop any stale copy
# a previously-run app cached) so `import secrets` reads THAT file, not the
# /secrets.py on the internal flash.
...
REGION = _sec("REGION", "eu")
TIMEZONE = _sec("TIMEZONE", 0)
WIFI_SSID = _sec("WIFI_SSID")
WIFI_PASSWORD = _sec("WIFI_PASSWORD")
...
def update():
    ...
    if clock_state == ClockState.Running:
        ...
        user_message("Please Wait", [f"Connecting to WiFi...{WIFI_SSID}"])
        if wifi.connect(ssid=WIFI_SSID, psk=WIFI_PASSWORD):
