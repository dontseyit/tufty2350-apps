...
class Apps:
    # apps live in category folders under root (e.g. user/, factory/). Categories
    # are shown in this order so the wearer's own apps land on the first page;
    # any other category folder is appended after these, alphabetically.
    CATEGORY_ORDER = ("user", "factory")
    ...
    def __init__(self, root):
        ...
        def add(rel):
            # rel is the app dir relative to root, e.g. "user/clock" - the menu
            # returns f"/system/apps/{rel}" so nested apps launch correctly.
            icon = f"{root}/{rel}/icon.png"
            if file_exists(icon):
                leaf = rel.rsplit("/", 1)[-1]
                name = " ".join([capitalize(w) for w in leaf.split("_")])
                App(self.apps, name, rel, image.load(icon))

        entries = [e for e in sorted(os.listdir(root)) if is_dir(f"{root}/{e}")]
        # an entry with its own icon.png is a (loose) app; one without is a
        # category folder we recurse into one level (menu/ has no icon -> skipped)
        loose = [e for e in entries if file_exists(f"{root}/{e}/icon.png")]
        cats = [e for e in entries if not file_exists(f"{root}/{e}/icon.png")]
        cats = ([c for c in self.CATEGORY_ORDER if c in cats]
                + [c for c in cats if c not in self.CATEGORY_ORDER])

        for cat in cats:
            for sub in sorted(os.listdir(f"{root}/{cat}")):
                if is_dir(f"{root}/{cat}/{sub}"):
                    add(f"{cat}/{sub}")
        for app in loose:              # any app dropped straight into root
            add(app)
